            ...
            <div v-if="show" class="">
                <div class="bg-white rounded shadow-sm pb-3 ">
                    <div class="pb-1 mb-0 d-flex">
                        <a href="/documents" class="text-center btn btn-outline-primary btn-block mt-2 mr-1">
                            <h3>{{propertyDocuments}}</h3>
                            <small>Properties</small>
                        </a>
                        <a href="/documents" class="text-center btn btn-outline-primary btn-block mt-2 mr-1">
                            <h3>{{tenantDocuments}}</h3>
                            <small>Tenants</small>
                        </a>
                    </div>
                </div>
...
export default {
    ...
    async mounted(){
        ...
        await axios.get('/getDocumentCounts').then(response => {
            if (response.data !== 'Err') {
                this.propertyDocuments = response.data.propertyCount;
                this.tenantDocuments = response.data.tenantCount;
            }
        });
